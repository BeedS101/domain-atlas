// Manual end-to-end check for item reissue (SPEC.md §5.1.1 — an item
...
// processItemUpdates()/checkAllMail()).
//
// Checks:
//   1. A visitor holds a real, issuer-signed Bronze Compass
//      (domain-atlas-item/1.0, atlas.wearable) from Example Plaza.
//   2. The issuer reissues that exact item server-side — the demo/admin
//      surface for this is POST /atlas/item/reissue, a real endpoint, not
...
//      small badge (itemUpdatesBadge) reflects this until the tab is
//      opened.
...
(async () => {
  const userDataDir = path.resolve(__dirname, '.chrome-profile-item-update-check');
  ...
  try {
    ...
    console.log('STEP 1: issuer reissues that exact item server-side (POST /atlas/item/reissue) — the demo/admin trigger');
    const reissue1 = await postJson(8001, '/atlas/item/reissue', {
      credential: held1,
    ...
    const reissue2 = await postJson(8001, '/atlas/item/reissue', {
      credential: held2,
