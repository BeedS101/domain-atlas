...
**Status:** Draft v1.21 (informational, pre-standardization)
**Date:** 2026-09-20
...
`proof` covers `{offer, want, from, expiresAt}`, plus `counterparty` on a claim (below), when it's present. A visitor presents this intent alongside a fungible balance (§5.4) sufficient to cover `offer` — the station confirms `asset.fungible === true` and `asset.tradeScope !== "bound"` on it as part of the same check §5.4 already requires before honoring any split or consolidation, and rejects the trade otherwise; the station verifies this independently before any match or settlement, the same possession check §5 step 3 already requires for any other presentation.
...
**Catalog discovery.** Posting a listing means naming a `want`, and naming a `want` means already knowing what a station is willing to mint at all — before this, a client had no way to learn that except by hardcoding it out of band. A station exposes its own catalog of tradable fungible classes for anyone to read, the same ungated "read is open" treatment Browsing gets just above: each entry names its `class`, a display `name`, an optional `thumbnail`, and its `tradeScope` (§5), filtered to `fungible === true` and `tradeScope !== "bound"` — the same two conditions a presented balance must already satisfy to be traded at all, so nothing offered here could fail that check later. This is deliberately scoped to a station's own catalog, describing only what that one domain itself is willing to mint; a client composing a trade against a *different* domain's station — fetching a foreign catalog live, filtered to only that domain's `tradeScope: "global"`-flagged classes (§5) — is a natural extension of this same read, not a different mechanism, but is left unspecified here pending real cross-domain settlement (below).

**Claiming.** A visitor fulfills one specific listing by presenting their own signed intent (mirroring the listing's `want` as their own `offer` and vice versa, naming the poster's public key in `counterparty`), their own presented balance, and their own Trading Station membership, alongside the target listing's id. The station verifies all three the same way it verified the original posting, confirms the named listing is still open and unexpired, confirms the claimant's intent truly mirrors it, and only then settles: it issues fresh balance credentials to both parties, each `supersedes`-ing what they held before (§5.4), and revokes the pre-trade balances in the same atomic act. Both sides settle together or neither does — a station that only ever received the original posting, with no claim arriving before it expires, holds an unmatched listing that lapses and changes nothing; there's no partial-trade state visible outside the station. A claim against a listing that's already gone (claimed by someone else first, expired, or withdrawn) fails cleanly; nothing about presenting a stale claim can ever double-settle a listing that's already resolved.
